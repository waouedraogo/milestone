 <!DOCTYPE html>
<html>
 <div class="row">
      <div class="col-md-3">
      
        <div class="panel panel-default">
          <div class="panel-heading">Account</div>
              <div class="panel-body">
                  <ul class="nav nav-pills nav-stacked">
                      <li><a href="#">Login</a></li>
                      <li class="active"><a href="../registration.php">Register</a></li>
                      <li class="active"><a href="#">Password Recovery</a></li>
                      <li class="active"><a href="#">My Account</a></li>
                      <li class="active"><a href="admin.php">Admin Access</a></li>
                      <li class="active"><a href="#">Show All Members</a></li>
                      <li class="active"><a href="#">Show Contacs Us Inquires</a></li>
                      <li class="active"><a href="#">Add Member Account</a></li>
                      <li class="active"><a href="#">Update Member Account</a></li>
                      <li class="active"><a href="#">Delete Member Account</a></li>
                  </ul>  
              </div>
        </div>      
          
      </div>
</html>